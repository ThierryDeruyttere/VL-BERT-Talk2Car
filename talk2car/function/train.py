...
from common.metrics import talk2car_metrics
...
from talk2car.data.build import make_dataloader, build_dataset, build_transforms
from talk2car.modules import *
from talk2car.function.val import do_validation
import copy
...
def train_net(args, config):
    ...
    if config.NETWORK.PARTIAL_PRETRAIN != "":
        ...
        #     pretrain_state_dict = torch.load(config.NETWORK.PARTIAL_PRETRAIN[0], map_location=lambda storage, loc: storage)['state_dict']
        #     pretrain_state_dict_ = torch.load(config.NETWORK.PARTIAL_PRETRAIN[1], map_location=lambda storage, loc: storage)['state_dict']
        #     for k, v in pretrain_state_dict_.items():
        #         if k.startswith('module.vlbert.encoder.layer.') :
        #             k_split = k.split('.')
        #             new_layer = str(int(k_split[4]) + 48)
        #             new_key = k.replace(k_split[4], new_layer, 1)
        #             pretrain_state_dict[new_key] = v
        #         elif k.startswith('vlbert.encoder.layer.'):
        #             k_split = k.split('.')
        #             new_layer = str(int(k_split[3]) + 48)
        #             new_key = k.replace(k_split[3], new_layer, 1)
        #             pretrain_state_dict[new_key] = v
        # else:
        ...
        if len(prefix_change) > 0:
            ...
            for k, v in pretrain_state_dict.items():
                if k.startswith('module.vlbert.encoder.layer.') or k.startswith('vlbert.encoder.layer.'):
                    if k.startswith('module.vlbert.encoder.layer.'):
                        ind = 4
                    else: 
                        ind = 3
                    k_split = k.split('.')
                    new_layer = str(int(k_split[ind]) + 24)
                    new_key = k.replace(k_split[ind], new_layer, 1)
                    no_match = True
                    for pretrain_prefix, new_prefix in prefix_change:
                        if new_key.startswith(pretrain_prefix):
                            new_key = new_prefix + new_key[len(pretrain_prefix):]
                            pretrain_state_dict_parsed[new_key] = copy.deepcopy(v)
                            no_match = False
                            break
                    if no_match:
                        pretrain_state_dict_parsed[new_key] = copy.deepcopy(v)
    ...
    # metrics
    train_metrics_list = [talk2car_metrics.RefAccuracy(allreduce=args.dist,
                                                      num_replicas=world_size if args.dist else 1),
                          talk2car_metrics.ClsAccuracy(allreduce=args.dist,
                                                      num_replicas=world_size if args.dist else 1),
                          talk2car_metrics.ClsPosAccuracy(allreduce=args.dist,
                                                         num_replicas=world_size if args.dist else 1),
                          talk2car_metrics.ClsPosFraction(allreduce=args.dist,
                                                         num_replicas=world_size if args.dist else 1),
                          ]
    val_metrics_list = [talk2car_metrics.RefAccuracy(allreduce=args.dist,
                                                    num_replicas=world_size if args.dist else 1)]
    for output_name, display_name in config.TRAIN.LOSS_LOGGERS:
        train_metrics_list.append(
            talk2car_metrics.LossLogger(output_name, display_name=display_name, allreduce=args.dist,
                                       num_replicas=world_size if args.dist else 1))
